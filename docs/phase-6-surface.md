...
|---|---|---|
| **`table_copy.wast` cross-module funcref-in-`elem`-segment init** — the verifier imports module `a`'s functions and initialises `elem` segments with `ref.func` of those *imported* functions, then dispatches via `call_indirect` | **~1,088** | Categorized-deferred — a **deeper** cross-module feature than the `CallImport` direct dispatch this phase landed (S11). `table_copy`'s same-module + multi-table `call_indirect` asserts (569) **pass**; the honest accounting is "569 pass / 1,088 residual", never the impossible "1,649 flip". |
