...
fn wrap(body: CExpr) -> CModule {
  CModule(name: "twocore@lazymask@t", exports: [], attributes: [], defs: [
...
pub fn compiled_original_and_rewritten_agree_test() {
  ...
  let orig_mod = load(mask_mod("twocore@lazymask@orig", original))
  let rewr_mod = load(mask_mod("twocore@lazymask@rewr", rewritten(original)))
