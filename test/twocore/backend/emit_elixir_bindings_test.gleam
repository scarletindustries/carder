...
//// toolchain** (`twocore_bindings_ffi:compile_load_elixir`), and calls typed exports through the
//// compiled binding — asserting the native result equals BOTH the WASM-spec value AND the raw
//// in-process `.beam` oracle (`twocore_threaded_test_ffi`). Coverage: a signed-int export, a
...
//// `PATH` (`twocore_bindings_ffi:which/1`). When absent (as on CI, which does NOT install Elixir)
...
////
//// The binding module base names here (`twocore@wasm@xstate` / `twocore@wasm@xpure`) are
...
fn run_compile_call() -> Nil {
  ...
  let a_atom = atom.create("Elixir.TwocoreWasmXstateBindings")
  let b_atom = atom.create("Elixir.TwocoreWasmXpureBindings")
  let assert Ok(_) =
...
pub fn emits_single_named_file_test() {
  ...
  assert list.map(files, fn(f) { f.path })
    == ["twocore_wasm_xstate_bindings.ex"]
  let assert [gf] = files
  assert string.contains(gf.content, "defmodule TwocoreWasmXstateBindings do")
  ...
  assert string.contains(gf.content, ":\"twocore@wasm@xstate\".instantiate()")
  assert string.contains(gf.content, ":\"twocore@wasm@xstate\".add(inst,")
...
pub fn stateless_shape_test() {
  ...
  assert string.contains(c, "st = :\"twocore@wasm@xpure\".instantiate()")
}
