//// Correctness of the A4 chunked-compile split (`twocore/backend/chunk`).
////
...
import gleam/string
import simplifile
import twocore/conformance/corpus.{Returns, Traps}
...
fn diff_program(name: String) -> List(String) {
  let base = "test/twocore/conformance/corpus/" <> name
...
pub fn below_threshold_is_single_chunk_test() {
  let assert Ok(wasm) =
    simplifile.read_bits("test/twocore/conformance/corpus/intops.wasm")
