...
//// compiler** (`twocore_bindings_ffi:compile_load_erlang` → `compile:file`), and calls typed
//// exports through the compiled binding — asserting the native result equals BOTH the WASM-spec
//// value AND the raw in-process `.beam` oracle (`twocore_threaded_test_ffi`). Coverage: a signed-int
...
////
//// The binding module base names here (`twocore@wasm@estate` / `twocore@wasm@epure`) are
...
pub fn erlang_binding_compile_call_test() {
  ...
  let a_atom = atom.create("twocore_wasm_estate_bindings")
  let b_atom = atom.create("twocore_wasm_epure_bindings")
  let assert Ok(_) =
...
pub fn emits_single_named_file_test() {
  ...
  assert list.map(files, fn(f) { f.path })
    == ["twocore_wasm_estate_bindings.erl"]
  let assert [gf] = files
  assert string.contains(gf.content, "-module(twocore_wasm_estate_bindings).")
  // Remote dispatch targets the exact (single-quoted) loaded `.beam` module atom.
  assert string.contains(gf.content, "'twocore@wasm@estate':instantiate()")
...
pub fn stateless_shape_test() {
  ...
  assert string.contains(c, "St = 'twocore@wasm@epure':instantiate(),")
}
