//// Milestone 0 (specs/HANDOFF-arc-frontend.md §7) — de-risk the arc→2core perf premise.
////
//// Hand-constructs the `ir.Module` a JS→2core emitter WOULD produce for two hot kernels
...
//      `NumTerm(NAdd)` accumulates. This is exactly what emit_2core would emit for the
...
pub fn milestone0_bench_test() {
  ...
  // Compiled 2core IR → BEAM (times `repeat` invocations, `n` iterations each).
