//// Phase 11 · P11-04 — tests for the `--link` pre-link fail-closed gate (`twocore.link_gate/2`).
////
...
pub fn gate_rejects_nif_tier_test() {
  ...
  assert twocore.link_gate(binding, module_with_imports([]))
    == Error(twocore.LinkTierNif)
...
pub fn gate_rejects_import_bearing_test() {
  ...
  assert twocore.link_gate(profiles.safe(), fn_import)
    == Error(twocore.LinkImportBearing(1))

  let global_import =
    module_with_imports([ir.ImportGlobal("env", "g", ir.TI32, False)])
  assert twocore.link_gate(profiles.safe(), global_import)
    == Error(twocore.LinkImportBearing(1))
}
...
pub fn gate_admits_import_free_tier_po_test() {
  ...
  assert twocore.link_gate(profiles.safe(), module_with_imports([])) == Ok(Nil)
  ...
  assert twocore.link_gate(tier_o, module_with_imports([])) == Ok(Nil)
}
