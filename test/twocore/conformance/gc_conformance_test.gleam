...
//// `.wast` suite through the full 2core pipeline (decode → validate → lower → emit → BEAM) and
...
//// GC engine 2core added: struct/array/i31 allocation + access lowered onto a per-process arena of
...
const gc_fixtures_dir = "test/twocore/conformance/fixtures/gc"
...
fn print_report(results: List(#(String, String, Report))) -> Nil {
  io.println(
    "\n=== Phase-8 GC-engine conformance (official GC .wast → 2core → BEAM, safe/unsafe [Cell]) ===",
  )
...
fn announce_absent() -> Nil {
  io.println(
    "\n[gc-conformance] no fixtures/gc present; run test/twocore/conformance/vendor/vendor.sh (needs wasm-tools)",
  )
}
