...
//// recovered into Gleam by `twocore_rt_exn_test_ffi` (pure Gleam cannot `catch`), where the real
...
@external(erlang, "twocore_rt_exn_test_ffi", "caught_reason")
...
@external(erlang, "twocore_rt_exn_test_ffi", "caught_class")
fn caught_class(action: fn() -> a) -> Dynamic

/// Run `action` (which must raise) and return the caught exception's Stacktrace.
@external(erlang, "twocore_rt_exn_test_ffi", "caught_stack")
fn caught_stack(action: fn() -> a) -> Dynamic

/// From `twocore_rt_test_ffi` (unit 09): `Ok(kind)` iff `action` raises `error:{wasm_trap, Kind}`,
...
/// D3a (grep-backed): `rt_exn.gleam` + `twocore_rt_exn_ffi.erl` contain NO `apply`, NO
...
pub fn no_ambient_authority_d3a_test() {
  let assert Ok(gleam_src) = simplifile.read("src/twocore/runtime/rt_exn.gleam")
  let assert Ok(erl_src) = simplifile.read("src/twocore_rt_exn_ffi.erl")
