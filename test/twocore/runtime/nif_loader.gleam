...
//// (`c_src/twocore_rt_mem_nif.c`) is built + `load_nif`'d ONCE and REUSED everywhere the native
...
////
//// `load_nif` is VM-GLOBAL: it attaches C to the `twocore_rt_mem_nif_ffi` shim atom, so ANY test that
...
////
//// gleeunit runs test modules in `filelib:wildcard` (sorted-by-path) order, so `twocore/conformance/*`
//// runs BEFORE `twocore/runtime/rt_mem_nif_build_test` (the S15-01 keystone probe) — which force-reloads
...
//// resource of that type is live — and the conformance harness (`twocore_conformance_ffi:start_common`)
...
/// Compile the committed `c_src/twocore_rt_mem_nif.c` + `load_nif` it into the shim (the S15-01 build
...
/// `twocore/runtime/rt_state:clear/0` — drop this process's seeded cell (its memory handle) so a native
/// reload is not blocked by a resource this caller left live.
@external(erlang, "twocore@runtime@rt_state", "clear")
fn clear_state() -> Nil
