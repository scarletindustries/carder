...
//// (`porf run != node`, a Porffor bug we do not hold 2core to, §E.2). A program with `skip: None`
...
//// Porffor's broken lexical-closure capture -> `NaN`/`ReferenceError`). 2core reproduces `porf run`
//// byte-for-byte on ALL 55 (including the 3 divergent ones), so `fail == 0` and the residual is
//// bounded by Porffor, not 2core (J8).

import gleam/option.{type Option, None, Some}
import twocore/js/report.{type SkipCategory, PorfforVsNodeDivergence}

const root = "test/twocore/js/corpus/"
