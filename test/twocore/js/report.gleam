...
//// compiled by Porffor and run through 2core on the BEAM, produce output byte-identical to
...
///   Porffor's ~1/3-of-ECMA coverage (J8). NOT a 2core gap.
...
/// - `TwocoreGap(stage)` — decode/validate/lower/emit/build REJECTED a construct 2core does not
...
///   2core to (2core still reproduces Porffor's wasm faithfully — beam == porf — but "pass" is
...
pub type SkipCategory {
  ...
  TwocoreGap(stage: String)
  PorfforVsNodeDivergence(note: String)
  ReferenceUnavailable
}

/// The verdict for a single corpus program (§A.1). Exactly one of:
/// - `Pass` — Porffor compiled it, 2core ran it on the BEAM, the BEAM console output is
///   byte-identical to `porf run` AND the reference is itself correct JS (`porf run == node`).
/// - `Fail(reason)` — Porffor compiled it + 2core ran it, but the BEAM output/outcome DIVERGED
///   from Porffor's own execution — a real 2core bug (the target of the whole harness).
...
pub fn skip_label(category: SkipCategory) -> String {
  case category {
    ...
    TwocoreGap(stage) -> "TwocoreGap (" <> stage <> ")"
    PorfforVsNodeDivergence(note) -> "PorfforVsNodeDivergence (" <> note <> ")"
    ReferenceUnavailable -> "ReferenceUnavailable"
  }
}

/// The one-word bucket name for the histogram tally (`Uncompilable`/`Unprovided`/`2coreGap`/
...
pub fn render(report: JsReport) -> String {
  ...
  let header =
    ...
    <> "  2coreGap="
    <> int.to_string(count_bucket(report, "2coreGap"))
    <> "  PorfVsNode="
