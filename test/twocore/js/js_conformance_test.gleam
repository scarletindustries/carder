...
////
//// Drives the whole JS corpus (`corpus.programs()`) through Porffor -> 2core -> BEAM under
//// `profiles.porffor()` — each program's vendored `.wasm` (Porffor 0.61.13's output) run through
//// the FULL 2core pipeline via `pipeline.run_porffor` — and JUDGES each program's captured console
...
//// 2core reproduces `porf run` byte-for-byte even on those three, so they are Porffor's bound,
//// not 2core's — J8).
...
import simplifile
import twocore/js/corpus.{type Program, Threw}
import twocore/js/report.{
  type Verdict, Fail, Pass, ReferenceUnavailable, Skip, TwocoreGap,
  UnprovidedIntrinsic,
}
import twocore/pipeline.{type PorfforRun}

/// Judge one program's 2core run against its baked `porf run` reference (§E, the single comparison
/// authority). `beam` is 2core's `PorfforRun`; `expected` is the baked stdout bytes.
///
...
///   either is a `Fail` (a real 2core bug) REGARDLESS of the program's `skip` field — 2core must
...
/// Run one program through 2core and judge it, or categorize a pre-run failure. A missing vendored
/// `.wasm`/`.expected` is `Skip(ReferenceUnavailable)` (cannot judge); a `run_porffor` pipeline
/// rejection (decode/validate/lower/emit/build) is `Skip(TwocoreGap(_))` — a named 2core gap, never
/// a false green. Total.
  ...
  case wasm_res, exp_res {
    Ok(wasm), Ok(expected) ->
      case pipeline.run_porffor(wasm, "m") {
        ...
        Error(e) -> Skip(TwocoreGap(string.inspect(e)))
      }
...
pub fn js_runs_on_beam_and_matches_porffor_test() {
  ...
  // (a) no 2core divergence from Porffor's own execution.
