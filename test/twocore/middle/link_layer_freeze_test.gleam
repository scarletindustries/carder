...
//// `src/twocore/runtime/` imports a compiler layer (`twocore/frontend`, `twocore/middle`, or
//// `twocore/backend`). `--link` (P11-03) merges the runtime's transitive dependency closure into a
...
//// `twocore/middle/ir_opt` solely for the `OptLevel` enum, which P11-01 relocated to the leaf
//// `twocore/opt_level`. These tests encode the *invariant*, not the current file contents: they
...
//// through the one non-runtime, non-compiler `twocore` module the runtime references — `twocore/ir`.
...
/// module roots — or any submodule beneath one (`twocore/middle/ir_opt`, `twocore/backend/emit_core`,
...
///   `"  import twocore/ir.{type Module}"` yields `Ok("twocore/ir")` and
...
///   comment that merely *names* a module (e.g. `//// …twocore/middle…`) is never misread as an
///   import. Never panics.
...
/// - Return: every imported module path (e.g. `["gleam/list", "twocore/ir", …]`); the empty list
...
/// **The freeze (O1 "Clean layering").** No module under `src/twocore/runtime/` imports any compiler
...
/// (`instance`/`profiles` importing `twocore/middle/ir_opt`) and will fail again on any regression.
pub fn runtime_reaches_zero_compiler_modules_test() {
  let assert Ok(files) = simplifile.get_files("src/twocore/runtime")
...
/// **R2 lock — `twocore/ir` is a clean leaf.** Asserts `ir.gleam` imports *exactly* `gleam/list` +
...
pub fn ir_is_a_clean_leaf_test() {
  let assert Ok(src) = simplifile.read("src/twocore/ir.gleam")
