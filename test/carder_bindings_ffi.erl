%%% carder_bindings_ffi — the shared compile+call harness for the Phase-12 typed
...
%%% boundary of `src/carder_*_ffi.erl` — it only uses `os`, `file`/`filelib`, `compile`,
%%% `beam_lib`, `code`, `unicode`, and `erlang`/`erlang:open_port`. Namespace: hand-written
%%% Erlang under `test/`, so it carries the `carder_` prefix (same convention as the other
%%% `test/carder_*_ffi.erl` shims). It touches no unit-owned source file and is imported by
...
%%                            e.g. `src/carder_rt_state_ffi.erl`).
which(Exe) ->
...
compile_load_erlang(Files, Main) ->
    Dir = fresh_dir("carder_bindings_erl"),
...
compile_load_gleam_1(Exe, Files, Main) ->
    Proj = "carder_binding_probe",
    Dir = fresh_dir("carder_bindings_gleam"),
...
compile_load_elixir_1(Exe, Files, Main) ->
    Dir = fresh_dir("carder_bindings_elixir"),
...
%%
%% Params: `Mod :: atom()` (the binding module atom, e.g. `carder_wasm_x_bindings`).
...
%% timeout, with the port force-closed). Mirrors the port pattern in `carder_linked_boot_ffi`.
