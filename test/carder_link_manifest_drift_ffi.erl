...
-module(carder_link_manifest_drift_ffi).
...
%% lists of UTF-8 binaries: the in-closure `carder@runtime@*` modules, the in-closure `gleam@*`
...
closure(RootBinaries) ->
    ...
    Runtime = sort_bins([M || M <- ClosureMods, has_prefix("carder@runtime@", M)]),
    Gleam = sort_bins([M || M <- ClosureMods, has_prefix("gleam@", M)]),
    Ffi = sort_bins([M || M <- ClosureMods,
                          not has_prefix("carder@", M),
