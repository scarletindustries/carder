%% Test-only FFI shim for carder's backend test harness.
%%
%% Hand-written Erlang, so it carries the `carder_` namespace prefix (overview §5).
%% It lives under `test/` and exists ONLY to give the Gleam harness the few host
%% capabilities Gleam's stdlib does not provide: invoking a freshly-loaded generated
%% module while catching a trap, running one instance in its own process over the
%% run-ABI, reading corpus files from disk, and probing a process's live memory.
%% It touches no unit-owned source file.
%%
%% The module atom MUST stay distinct from the wasm frontend package's own shim
%% (`carder_conformance_ffi`): two `.erl` files sharing a module atom across packages
%% produce NO error and NO warning — one silently shadows the other.
%%
%% NOTE (the frontend split): the wast2json/Tier-B helpers this shim used to carry
%% (`parse_json/1`, `list_dir/1`, `find_executable/1`, `run/2`) and the
%% `assert_invalid` routing spy (`spy_reset/0`, `spy_mark/0`, `spy_called/0`) went to
%% scribbler with the WebAssembly conformance suite — carder has no `.wast` fixtures,
%% no reference-engine tier, and no frontend-rejection partition to police.
-module(carder_harness_ffi).
-export([catch_apply/3, read_file/1, unique_int/0,
...
%% corpus `.ir` programs and their `.expected` oracles.
read_file(Path) ->
