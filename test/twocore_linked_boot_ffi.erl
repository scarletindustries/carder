%%% twocore_linked_boot_ffi — the bare-node isolation boot harness (Phase 11 · P11-05).
%%%
%%% TRUST BOUNDARY: this is TEST-ONLY infrastructure. It is NOT the OTP-compiler-
%%% internals trust boundary of `src/twocore_linker_ffi.erl`/`twocore_codegen_ffi.erl`
...
%%%
%%% Namespace: hand-written Erlang under `test/`, so it carries the `twocore_` prefix
%%% (same convention as the seven existing `test/twocore_*_ffi.erl` shims). It touches
...
-module(twocore_linked_boot_ffi).
...
%% `twocore@runtime@*` runtime module, one `gleam@*` module, the hand-written
%% `gleam_stdlib` FFI, and the shared `twocore@ir` leaf — a cross-section of every
...
%% stub `twocore@` leak module) at test time.
%%
%% Params:
...
compile_source(ModuleName, Source) ->
    Dir = fresh_dir("twocore_boot_src"),
...
%%       parent test VM's `…/twocore/ebin` `twocore@*`/`gleam@*` beams would otherwise
...
%%                                          `twocore@` module here to fire the gate.
...
boot_invoke_1(Exe, Beam, ModuleName, Fun, Args, Extra) ->
    LinkDir = fresh_dir("twocore_boot_link"),
...
write_extra([]) ->
    {[], []};
write_extra(Extra) ->
    ExtraDir = fresh_dir("twocore_boot_extra"),
...
%%       `twocore_conformance_ffi:start_common/2`.
...
%% `twocore_conformance_ffi:collect/2`, plus the bound.
...
%% — the repo-wide convention, e.g. `src/twocore_rt_state_ffi.erl`. `{error, nil}` is used.)
which_in_parent(Mod) ->
