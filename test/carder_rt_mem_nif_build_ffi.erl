%%% carder_rt_mem_nif_build_ffi — the toolchain-gated tier-N NIF build+load harness (S15-01 keystone).
%%%
%%% TRUST BOUNDARY: TEST-ONLY infrastructure. Like `carder_bindings_ffi`, it is NOT the
%%% OTP-compiler-internals trust boundary of `src/carder_*_ffi.erl`; it only uses `os`,
%%% `file`/`filelib`, `code`, `unicode`, and `erlang:open_port`. Hand-written Erlang under `test/`, so
%%% it carries the `carder_` prefix (same convention as the other `test/carder_*_ffi.erl` shims). It
...
%%% `cc`-gated `cc -shared -fPIC …` into a tempdir `.so`, force-reloads the `carder_rt_mem_nif_ffi`
%%% shim so `-on_load` attaches it, and calls a NIF through it. Modelled on `carder_bindings_ffi`'s
...
%%% `c_src/carder_rt_mem_nif.c` at test time):
...
%%%   compile_load_cnif/0  — compile the COMMITTED `c_src/carder_rt_mem_nif.c` + load (downstream).
%%%
...
-module(carder_rt_mem_nif_build_ffi).
...
-define(SHIM, carder_rt_mem_nif_ffi).
-define(SO_BASE, "carder_rt_mem_nif").
...
%% Locate executable `Exe` on `PATH` (the toolchain gate, `carder_bindings_ffi:52-56`).
%%
...
%% `c_src/carder_rt_mem_nif.c` S15-02's) against the committed `c_src/carder_rt_mem_nif.h`, then load
...
%% Compile the COMMITTED `c_src/carder_rt_mem_nif.c` (owned by S15-02) against the committed
%% `c_src/carder_rt_mem_nif.h`, then load + verify. Called by S15-02/03/04's per-op differential; at
%% keystone time the `.c` does not exist yet, so this returns a clear `{build_error, _}` (never called
%% by the keystone's own tests).
%%
%% Returns: `loaded` | `skip_no_toolchain` | `{build_error, binary()}`.
compile_load_cnif() ->
    case read_repo_file("c_src/carder_rt_mem_nif.c") of
        {ok, CSource} ->
            compile_and_load(CSource);
        {error, _} ->
            {build_error,
                <<"c_src/carder_rt_mem_nif.c not found (owned by S15-02, not built this unit)">>}
    end.
...
compile_and_load(CSource) ->
    case cc() of
        {error, nil} ->
            ...
            skip_no_toolchain;
        {ok, CcBin} ->
            case read_repo_file("c_src/carder_rt_mem_nif.h") of
                {error, _} ->
                    {build_error, <<"c_src/carder_rt_mem_nif.h not found">>};
                {ok, Header} ->
                    Cc = binary_to_list(CcBin),
                    Dir = fresh_dir("carder_rt_mem_nif_build"),
...
reload_and_verify(Base) ->
    os:putenv("CARDER_RT_MEM_NIF_SO", Base),
    _ = code:purge(?SHIM),
    _ = code:delete(?SHIM),
...
%% and declares `ERL_NIF_INIT(carder_rt_mem_nif_ffi, …)`. One compile therefore proves the WHOLE
...
probe_c_source() ->
    <<
        "#include <erl_nif.h>\n"
        "#include \"carder_rt_mem_nif.h\"\n"
        "\n"
        "static ErlNifResourceType *CARDER_MEM_RT = NULL;\n"
        "\n"
        "static int load(ErlNifEnv *env, void **priv_data, ERL_NIF_TERM load_info) {\n"
        "    (void)priv_data; (void)load_info;\n"
        "    ErlNifResourceType *rt = enif_open_resource_type(\n"
        "        env, NULL, CARDER_RT_MEM_NIF_RESOURCE, NULL, ERL_NIF_RT_CREATE, NULL);\n"
        "    if (rt == NULL) return -1;\n"
        "    CARDER_MEM_RT = rt;\n"
        "    return 0;\n"
        "}\n"
        ...
        "};\n"
        "\n"
        "ERL_NIF_INIT(carder_rt_mem_nif_ffi, nif_funcs, load, NULL, NULL, NULL)\n"
    >>.

%% ─────────────────────────── generic helpers (mirroring carder_bindings_ffi) ───────────────────────────

%% Read a repo file at `RelPath` relative to the test cwd (the project root — `gleam test` runs there,
%% e.g. it reads `test/carder/conformance/corpus/…`). Returns `{ok, binary()}` | `{error, term()}`.
...
%% the port force-closed). The exact port pattern of `carder_bindings_ffi:277-292`.
