...
pub fn link_flag_rejected_on_non_link_verbs_test() {
  ...
  let assert Error(m3) = carder.run(["run", "--link", wasm, "add", "2", "3"])
  assert string.contains(m3, "--link")
  // R13 deferral: the .core-input verb has no binding/profile to gate on.
  let assert Error(m4) = carder.run(["to-beam", "--link", "x.core"])
...
pub fn linked_build_smoke_test() {
  ...
  let assert Ok(lmsg) =
    carder.run(["to-beam-wasm", "--unsafe", "--link", wasm, linked])
  ...
  let assert Ok(_) = carder.run(["to-beam-wasm", "--unsafe", wasm, unlinked])

  // exec both: the linked artifact returns the same spec-correct value as the non-linked one.
  let assert Ok(linked_out) = carder.run(["exec", linked, "add", "2", "3"])
  let assert Ok(plain_out) = carder.run(["exec", unlinked, "add", "2", "3"])
