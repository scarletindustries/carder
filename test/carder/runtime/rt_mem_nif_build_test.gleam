...
//// COMPILES into a loadable `.so`, `load_nif` ATTACHES it to the `carder_rt_mem_nif_ffi` shim, and a
...
/// Compile the embedded `nif_ping` probe `.c` (against the committed `c_src/carder_rt_mem_nif.h`),
/// force-reload the shim so `-on_load` attaches the fresh `.so`, and verify `nif_ping()` returns
/// `pong`. See `test/carder_rt_mem_nif_build_ffi.erl`. Returns the categorized `BuildResult`.
@external(erlang, "carder_rt_mem_nif_build_ffi", "compile_load_probe")
...
@external(erlang, "carder_rt_mem_nif_build_ffi", "which")
...
@external(erlang, "carder_rt_mem_nif_ffi", "nif_ping")
fn nif_ping() -> Atom
...
pub fn gate_categorizes_toolchain_absence_test() {
  which("carder-cc-that-does-not-exist-xyz")
