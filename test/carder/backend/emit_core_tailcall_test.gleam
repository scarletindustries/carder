...
// ── test-only FFI (shared `carder_emit_test_ffi`, see the e2e suite) ──────────────
...
pub fn threaded_direct_return_call_to_state_reaching_is_tail_apply_test() {
  ...
  let m =
    ir.Module(
      name: "carder@tc@threaded_direct",
      uses_numerics: True,
...
pub fn indirect_return_call_ok_arm_is_tail_apply_test() {
  ...
  assert tmod == "carder@runtime@rt_table"
...
pub fn import_return_call_routes_through_import_path_test() {
  ...
  assert lmod == "carder@runtime@link"
}
