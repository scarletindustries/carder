...
/// A minimal accepted-shape module `carder@wasm@<base>` (memory + a mutable global so it emits
...
pub fn bindings_without_threaded_rejected_test() {
  ...
  let assert Error(_) =
    simplifile.read_bits(dir <> "/carder@wasm@roundtrip.beam")
  let _ = simplifile.delete(dir)
}
...
pub fn folder_emission_and_beam_non_perturbation_test() {
  ...
  let beam_path = dir <> "/carder@wasm@roundtrip.beam"
...
pub fn emit_bindings_deterministic_test() {
  ...
  assert first == "carder@wasm@roundtrip.beam"
