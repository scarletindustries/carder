...
////
//// The Elixir arm is GATED on `elixirc` being on `PATH` (`carder_bindings_ffi:which`). CI does NOT
...
/// Assemble a numerics module named `carder@wasm@<base>` (R14) from `functions`, exporting each by
...
/// the two atoms are the loaded `.beam` module names (`carder@wasm@capstone` / `…capstonep`, R14).
///
...
/// Fully unload a binding module between per-language runs (R16 — see `carder_bindings_ffi:purge`).
@external(erlang, "carder_bindings_ffi", "purge")
...
@external(erlang, "carder_bindings_ffi", "is_loaded")
...
fn gleam_driver_source() -> String {
  "import carder_wasm_capstone_bindings as s
import carder_wasm_capstonep_bindings as p
...
/// module atom `carder_wasm_capstone_bindings`, so without the purge the second load would leave the
...
pub fn phase12_capstone_differential_test() {
  ...
  run_beam(
    matrix_name,
    stateless_name,
    "carder_wasm_capstone_bindings",
    "carder_wasm_capstonep_bindings",
    erlang_compile,
    oracle,
  )
  purge_shared_bindings()
  ...
  case which("elixirc") {
    ...
    Ok(_) -> {
      run_beam(
        matrix_name,
        stateless_name,
        "Elixir.CarderWasmCapstoneBindings",
        "Elixir.CarderWasmCapstonepBindings",
        elixir_compile,
        oracle,
      )
      // The Elixir binding atoms are DISTINCT (`Elixir.*`), so this purge is hygiene, not R16.
      purge_module(atom.create("Elixir.CarderWasmCapstoneBindings"))
      purge_module(atom.create("Elixir.CarderWasmCapstonepBindings"))
      Nil
    }
  }
}
...
fn purge_shared_bindings() -> Nil {
  purge_module(atom.create("carder_wasm_capstone_bindings"))
  purge_module(atom.create("carder_wasm_capstonep_bindings"))
  purge_module(atom.create("carder_wasm_capstone_bindings_ffi"))
  purge_module(atom.create("carder_wasm_capstonep_bindings_ffi"))
...
pub fn r16_shared_atom_clobber_and_purge_test() {
  case which("gleam") {
    ...
    Ok(_) -> {
      ...
      let binding = atom.create("carder_wasm_r16probe_bindings")
      ...
      purge_module(binding)
      purge_module(atom.create("carder_wasm_r16probe_bindings_ffi"))
      Nil
    }
  }
}
...
pub fn folder_driver_shipped_path_test() {
  ...
  assert paths
    == [
      dir_a <> "/carder@wasm@capstone.beam",
      dir_a <> "/carder_wasm_capstone_bindings.gleam",
      dir_a <> "/carder_wasm_capstone_bindings_README.md",
      dir_a <> "/carder_wasm_capstone_bindings_ffi.erl",
      dir_a <> "/carder_wasm_capstone_bindings.erl",
      dir_a <> "/carder_wasm_capstone_bindings.ex",
    ]

  // The written `.beam` is exactly the compiled artifact (the bindings are companions — P4).
  let assert Ok(written_beam) =
    simplifile.read_bits(dir_a <> "/carder@wasm@capstone.beam")
  ...
  assert read_text(dir_a, "carder_wasm_capstone_bindings.gleam")
    == read_text(dir_b, "carder_wasm_capstone_bindings.gleam")
  assert read_text(dir_a, "carder_wasm_capstone_bindings.erl")
    == read_text(dir_b, "carder_wasm_capstone_bindings.erl")
  assert read_text(dir_a, "carder_wasm_capstone_bindings.ex")
    == read_text(dir_b, "carder_wasm_capstone_bindings.ex")
...
fn scratch_dir(tag: String) -> String {
  "/tmp/carder_p12_capstone_" <> tag <> "_" <> int.to_string(unique_int())
}
