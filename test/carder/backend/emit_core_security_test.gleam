...
////
//// 1. Every inter-module `call` (`CCall`) targets a FIXED `carder@runtime@*` module
...
fn runtime_modules(b: instance.Binding) -> Set(String) {
  set.from_list([
    ...
    // through the already-admitted `link.call_import` (S5 — a homogeneous carder-only allow-set;
    // there is deliberately NO `erlang` entry, since no `erlang:apply` is emitted).
    "carder@runtime@rt_simd",
...
/// tightly-scoped loosening of the carder-only module homogeneity (S5), justified by the
...
fn assert_calls_are_runtime(m: CModule, binding: instance.Binding) {
  ...
  list.each(calls, fn(pair) {
    ...
    let ok = case mod_name {
      ...
      // every other `call` targets one of the binding's fixed carder runtime modules (S5).
...
/// `carder@runtime@*` module from the `Binding`, with a literal function atom — no
...
/// fixed `carder@runtime@*` modules; `St` is an ordinary argument (a Core var), never a
...
pub fn phase5_ops_have_no_ambient_authority_test() {
  ...
  list.each([cell, threaded, unsafe], fn(binding) {
    ...
    assert has_call(m, "carder@runtime@rt_ref", "is_null")
    ...
    assert has_call(m, "carder@runtime@link", "provided_memory_value")
    assert has_call(m, "carder@runtime@link", "provided_global_bits")
...
pub fn phase6_ops_have_no_ambient_authority_test() {
  ...
  list.each([cell, threaded, unsafe], fn(binding) {
    ...
    assert has_call(m, "carder@runtime@link", "call_import")
    ...
    // fault — `rt_trap:raise`.
    assert has_call(m, "carder@runtime@rt_simd", "i32x4_add")
    assert has_call(m, "carder@runtime@rt_simd", "i8x16_shuffle")
    ...
    assert has_call(m, "carder@runtime@link", "provided_func_call")
...
pub fn phase7_eh_ops_have_no_ambient_authority_test() {
  ...
  list.each([cell, threaded, unsafe], fn(binding) {
    ...
    assert has_call(m, "carder@runtime@rt_exn", "throw_exn")
    assert has_call(m, "carder@runtime@rt_exn", "match_tag")
    assert has_call(m, "carder@runtime@rt_exn", "is_wasm_exn")
    assert has_call(m, "carder@runtime@rt_exn", "reraise")
    assert has_call(m, "carder@runtime@rt_exn", "capture_exnref")
    assert has_call(m, "carder@runtime@rt_exn", "throw_ref")
  })
}
