...
/// The Phase-1 Safe binding (the fixed `carder@runtime@*` module table).
...
/// VERIFIED fact 1: the Gleam module `carder/runtime/rt_num` compiles to Erlang module
/// `carder@runtime@rt_num` and exports `i32_add/2`. The `Binding` already carries that
...
pub fn gleam_mangling_golden_test() {
  ...
  assert instance.safe_default().num_module == "carder@runtime@rt_num"
  assert function_exported(
      atom.create("carder@runtime@rt_num"),
      atom.create("i32_add"),
      2,
    )
    == True
...
pub fn num_op_name_matches_rt_num_test() {
  ...
  let m = atom.create("carder@runtime@rt_num")
...
/// pass-through — a static-type retag with NO `num_module` call. carder carries every scalar,
...
/// `carder@runtime@*` modules, only the codegen SHAPE differs (the strategy is the sole switch).
...
pub fn threaded_export_alias_forwards_at_plus_one_test() {
  ...
  let m =
    ir.Module(
      name: "carder@test@threadedalias",
      uses_numerics: True,
      memories: [ir.MemoryDecl(1, option.None, ir.Idx32)],
      globals: [],
      imports: [],
...
pub fn ref_is_null_test() {
  let assert FunDef(
    ...
    CFun(
      _,
      CCase(
        CCall(CAtom("carder@runtime@rt_ref"), CAtom("is_null"), [CVar("p0")]),
        [
...
const rt_simd_atom = "carder@runtime@rt_simd"
...
pub fn call_import_closure_dispatch_test() {
  ...
  assert has_call(body, "carder@runtime@link", "call_import")
...
pub fn call_import_seeds_func_import_vector_test() {
  ...
  assert has_call(inst_body, "carder@runtime@link", "provided_func_call")
}
...
const eh_exn_module = "carder@runtime@rt_exn"
...
pub fn try_lowers_to_ctry_with_tag_dispatch_test() {
  ...
  assert rec == rec2
  let assert [
    CClause([PTuple([PAtom("$2c_fall"), PVar(x)])], CAtom("true"), CVar(x2)),
    CClause([PTuple([PAtom("$2c_ret"), PVar(p)])], CAtom("true"), CVar(p2)),
  ] = arms
  assert x == x2
  assert p == p2
