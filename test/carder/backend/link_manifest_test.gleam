...
//// (`src/carder/backend/link_manifest.gleam`).
////
...
import carder/backend/link_manifest as m
import gleam/list
import gleam/set
...
/// runtime module (`carder@runtime@rt_num`) are all NOT ambient (fail-closed / in-closure ≠ ambient).
pub fn is_ambient_membership_test() {
  ...
  assert m.is_ambient("carder@runtime@rt_num") == False
}
...
/// `carder_rt_*_ffi` shims + `gleam_erlang_ffi` (the reconciliation's `.erl` sketch omitted the
...
pub fn ffi_bucket_shape_test() {
  ...
  let expected = [
    "gleam_stdlib", "carder_rt_exn_ffi", "carder_rt_mem_atomics_ffi",
    "carder_rt_ref_ffi", "carder_rt_state_ffi", "carder_rt_table_ets_ffi",
    "gleam_erlang_ffi",
  ]
  ...
  assert list.contains(ffi, "carder_rt_js_ffi") == False
  assert list.contains(ffi, "carder_cli_ffi") == False
  assert list.contains(ffi, "carder_codegen_ffi") == False
}
...
pub fn runtime_closure_excludes_offpath_modules_test() {
  ...
  assert list.contains(runtime, "carder@runtime@rt_js") == False
  assert list.contains(runtime, "carder@runtime@rt_mem_nif") == False
  assert list.contains(runtime, "carder@runtime@rt_bif") == False
  assert list.contains(runtime, "carder@runtime@instance") == False
  assert list.contains(runtime, "carder@runtime@profiles") == False
}
