...
//// `-pa` with no `carder@*`/`gleam@*` reachable; "Bare-node proof" — the artifact is
...
////   * NEGATIVE — with a `carder@` module deliberately on the child path, the in-child
...
/// manufacture self-contained fixtures (and the negative test's `carder@` leak stub).
@external(erlang, "carder_linked_boot_ffi", "compile_source")
...
/// injects a `carder@` module here). Returns `#(exit_status, combined_stdout_stderr)`.
...
/// `127` = no `erl`.
@external(erlang, "carder_linked_boot_ffi", "boot_invoke")
fn boot_invoke(
  beam: BitArray,
  module: Atom,
  function: Atom,
...
@external(erlang, "carder_linked_boot_ffi", "which_in_parent")
...
fn self_module() -> Atom {
  atom.create("carder_link_selftest_ok")
}

/// A genuinely self-contained Cell-ABI fixture: it calls ONLY `erlang:error/1` and `+`
/// (ambient BIFs on every OTP node) — no `carder@`/`gleam@` edge — so it runs on the
...
fn self_source() -> String {
  "-module(carder_link_selftest_ok).\n"
...
fn self_threaded_module() -> Atom {
  atom.create("carder_link_selftest_threaded")
}
...
fn self_threaded_source() -> String {
  "-module(carder_link_selftest_threaded).\n"
...
/// mergeable bucket (a `carder@runtime@*`, a `gleam@*`, the `gleam_stdlib` FFI, the
/// `carder@ir` leaf) is `Ok(_)` here. Closes the "misspelled gate ⇒ vacuous pass" hole.
pub fn gate_atoms_are_real_test() {
  let assert Ok(_) = which_in_parent(atom.create("carder@runtime@rt_mem"))
  let assert Ok(_) = which_in_parent(atom.create("gleam@list"))
  let assert Ok(_) = which_in_parent(atom.create("gleam_stdlib"))
  let assert Ok(_) = which_in_parent(atom.create("carder@ir"))
}
...
/// The "gate gates" proof (adversarial must-NOT). With a `carder@runtime@rt_mem` stub
...
pub fn gate_fires_when_carder_module_on_child_path_test() {
  let rt_mem = atom.create("carder@runtime@rt_mem")
  let assert Ok(stub) =
    compile_source(rt_mem, "-module('carder@runtime@rt_mem').\n-export([]).\n")
  let #(code, out) =
    boot_invoke(self_beam(), self_module(), atom.create("answer"), [], [
      #(rt_mem, stub),
    ])
