...
////
//// The introspection FFI (`test/carder_link_manifest_drift_ffi.erl`) is read-only: it reads
...
/// in-closure `carder@runtime@*`, in-closure `gleam@*`, in-closure hand-FFI `.erl`, and the
/// surviving remote (OTP-ambient) targets (module granularity).
@external(erlang, "carder_link_manifest_drift_ffi", "closure")
...
@external(erlang, "carder_link_manifest_drift_ffi", "mergeability_violations")
...
/// `carder@runtime@rt_mem_nif`. Proven against the RECOMPUTED closure, so a future edge that pulls
...
pub fn adversarial_exclusions_drift_test() {
  ...
  assert list.contains(closure, "carder@runtime@rt_mem_nif") == False
  assert list.contains(closure, "carder@runtime@rt_js") == False
}
