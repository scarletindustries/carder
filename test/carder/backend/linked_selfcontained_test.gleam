...
//// self-contained `.beam` — the generated module with its whole `carder@`/`gleam@`
...
////   `carder@`/`gleam@` reachable, and its value/trap must match the in-process oracle. Plus a
...
/// `carder@runtime@rt_simd` whose dispatch CAPTURES `fun carder@runtime@rt_num:*` values, and a
...
/// fun-capture (`fun 'carder@…'`/`fun 'gleam@…'` — all rewritten to self-module funrefs), and no
...
fn d3a_text_failures(name: String, text: String) -> List(String) {
  ...
  list.flatten([
    ...
    fail_if(
      string.contains(text, "fun 'carder@"),
      name <> ": residual off-closure carder@ fun-capture",
    ),
    fail_if(
...
/// `carder@`/`gleam@` reachable. Any leak (`LEAK:`), missing module (`NOLOAD:`), or value/trap
...
/// contract the in-child `code:which` gate HALTS before the invoke on any `carder@`/`gleam@` hit,
