...
// Test-only FFI (see `test/carder_emit_test_ffi.erl`): apply `M:F(Args)` and capture a
...
// Test-only FFI (see `test/carder_threaded_test_ffi.erl`): run the record-returning
...
/// `carder@runtime@*` modules; only the codegen shape differs).
...
@external(erlang, "carder_threaded_test_ffi", "instantiate_with")
...
pub fn ref_func_declarative_and_global_init_elem_e2e_test() {
  ...
  let m =
    ir.Module(
      name: "carder@e2e@reffunc_decl_globalinit",
      uses_numerics: True,
      memories: [],
...
@external(erlang, "carder_emit_test_ffi", "apply3")
...
/// are carder-compiled; B's `CallImport(0, …)` reads the closure from its seeded func-import vector
