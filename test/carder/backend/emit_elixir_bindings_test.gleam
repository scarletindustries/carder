//// P12-04 compile+call differential for the Elixir bindings emitter (`emit_elixir`).
////
...
//// them, `emit_elixir`s the bindings, compiles the emitted `.ex` with the **real `elixirc`
//// toolchain** (`carder_bindings_ffi:compile_load_elixir`), and calls typed exports through the
//// compiled binding — asserting the native result equals BOTH the WASM-spec value AND the raw
//// in-process `.beam` oracle (`carder_threaded_test_ffi`). Coverage: a signed-int export, a
//// multi-value tuple export, a trapping export (`{:error, {:wasm_trap, _}}`), a finite float
//// export, a non-finite (`+Inf`) float via the `{:nonfinite, _}`/raw-bits tagged tuple, an f32
//// single-rounding round-trip, a zero-result export, threaded state (two `bump`s accumulate; the
//// old instance is immutable), and the Stateless two-shape API (no `instantiate`, re-instantiating
//// internally).
////
//// **Elixir is best-effort (P8/R23):** the compile+call test is GATED on `elixirc` being on
//// `PATH` (`carder_bindings_ffi:which/1`). When absent (as on CI, which does NOT install Elixir)
//// it prints a categorized skip and PASSES — never a failure, never a false green. Locally
//// `elixirc` IS present, so the compile+call runs for real. The generated binding is written to
//// run with ZERO `Elixir.*` runtime deps (R23), so it executes on the bare BEAM node the suite
//// runs on (no Elixir stdlib ebin added to the path).
////
//// The pure unit tests (no toolchain) pin determinism, the single-file emission, the zero-Elixir-
//// dep property, and the Stateless vs Threaded `@spec` surface shapes (R19). They run always.
////
//// The binding module base names here (`carder@wasm@xstate` / `carder@wasm@xpure`) are
//// deliberately distinct from the Gleam/Erlang sibling tests' so nothing clashes in the shared
//// test VM. (The Elixir binding itself compiles to the DISTINCT atom `Elixir.*`, so R16 — the
//// cross-language binding-atom collision — does not apply to it.)
...
fn module_b() -> ir.Module {
  module_of("xpure", [add_fn(), fdiv_fn()])
}
...
// ───────────────────────────── compile+call FFI (real elixirc, best-effort) ─────────────────────────────

/// Probe a toolchain executable on `PATH`. `Ok(path)` present, `Error(Nil)` absent — the
/// best-effort gate for the Elixir arm (P8/R23).
...
/// The single `.ex` source of an emitted binding, as `#(basename, content)` for
/// `compile_load_elixir`. `emit_elixir` returns exactly one file.
fn source_file(desc: iface.Iface) -> #(String, String) {
  let assert [gf] = emit_elixir_bindings.emit_elixir(desc)
  #(gf.path, gf.content)
}

// ───────────────────────────── the compile+call differential (gated) ─────────────────────────────

/// THE headline: compile both emitted bindings with the real `elixirc` and call typed exports
/// through them, matching the WASM-spec value and the raw in-process `.beam` oracle.
///
/// **Best-effort (P8/R23):** gated on `elixirc` being on `PATH`. When absent (CI does not install
/// Elixir) this prints a categorized skip and RETURNS — a pass, never a failure. When present it
/// runs for real; compiles ONCE (both bindings in one `elixirc` spawn), then calls many exports.
pub fn elixir_binding_compile_call_test() {
  case which("elixirc") {
    Error(_) -> {
      io.println(
        "\n[p12-04] elixirc not on PATH — Elixir compile+call SKIPPED (categorized, best-effort P8/R23)",
      )
      Nil
    }
    Ok(_) -> run_compile_call()
  }
}

/// The real compile+call body, reached only when `elixirc` is present.
fn run_compile_call() -> Nil {
  ...
  // Compile both bindings (one elixirc spawn) and load their beams. The Elixir module atoms are
  // `Elixir.<Camelized base>Bindings`.
  let a_atom = atom.create("Elixir.CarderWasmXstateBindings")
  let b_atom = atom.create("Elixir.CarderWasmXpureBindings")
  let assert Ok(_) =
    compile_load_elixir([source_file(desc_a), source_file(desc_b)], a_atom)
  ...
  // ── Stateless two-shape API: no instantiate in the surface, re-instantiating internally ──
...
/// `emit_elixir` returns exactly ONE file (no companion catch shim — Elixir catches in-language),
/// named off the legalized module base, whose `defmodule` is the camelized alias. The remote
/// dispatch targets the exact (quoted) loaded `.beam` module atom, and the trap catch is STRUCTURAL
/// on `{:wasm_trap, _}` (R7), never a bare catch-all, never a `rescue` (R23).
pub fn emits_single_named_file_test() {
  let assert Ok(desc) = iface.describe(module_a(), profiles.portable())
  let files = emit_elixir_bindings.emit_elixir(desc)
  assert list.map(files, fn(f) { f.path }) == ["carder_wasm_xstate_bindings.ex"]
  let assert [gf] = files
  assert string.contains(gf.content, "defmodule CarderWasmXstateBindings do")
  // Remote dispatch targets the exact (quoted) loaded `.beam` module atom; a clean export name is
  // an unquoted call (hand-written-quality), the module atom is quoted (the `@` requires it).
  assert string.contains(gf.content, ":\"carder@wasm@xstate\".instantiate()")
  assert string.contains(gf.content, ":\"carder@wasm@xstate\".add(inst,")
  // The trap catch is STRUCTURAL on {:wasm_trap, _} (R7), never a bare catch-all, never a rescue.
  assert string.contains(
    gf.content,
    ":error, {:wasm_trap, _} = reason -> {:error, reason}",
  )
  assert !string.contains(gf.content, "rescue")
  assert !string.contains(gf.content, "catch\n      :error, reason ->")
}

/// R23 — the emitted binding has ZERO `Elixir.*` runtime deps: no `rescue`, no `import Bitwise`, no
/// struct, no `Enum`/`Integer`/`Float`/`Kernel` stdlib calls; integer masking is `:erlang.band`.
pub fn zero_elixir_runtime_deps_test() {
  let assert Ok(desc) = iface.describe(module_a(), profiles.portable())
  let #(_p, c) = source_file(desc)
  assert string.contains(c, ":erlang.band(")
  assert !string.contains(c, "import Bitwise")
  assert !string.contains(c, "defstruct")
  assert !string.contains(c, "rescue")
  assert !string.contains(c, "Enum.")
  assert !string.contains(c, "Integer.")
  assert !string.contains(c, "Float.")
  assert !string.contains(c, "Kernel.")
}

/// The Threaded surface (R19): an opaque `instance`, a `Result`-shaped `instantiate/0`, a
/// state-reaching export threading `instance()` back, and a pure export dropping it. Every function
/// carries an `@spec` + `@doc` (P5).
pub fn threaded_shape_test() {
  ...
  assert string.contains(c, "def instantiate() do")
  ...
  // Every export carries a @doc (P5).
  assert string.contains(c, "@doc \"Exported `add`")
}

/// The Stateless surface (R19, "the beautiful pure file"): NO `instance`, NO `instantiate`, each
/// export `f(args) :: {:ok, T} | {:error, trap()}` re-instantiating internally per call.
pub fn stateless_shape_test() {
  ...
  assert !string.contains(c, "def instantiate")
  assert string.contains(
    c,
  ...
  assert string.contains(c, "st = :\"carder@wasm@xpure\".instantiate()")
}
