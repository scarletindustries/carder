...
pub fn print_atom_matches_oracle_on_words_test() {
  [
    "ok", "erlang", "module", "let", "+", "-", "=<", "carder@runtime@rt_num",
...
pub fn mandatory_guard_and_value_list_pattern_test() {
  ...
  let m =
    CModule("carder@guard", [FName("f", 1)], [], [
      FunDef(
        FName("f", 1),
...
pub fn integration_add_compiles_loads_runs_test() {
  let m =
    CModule("carder@test@printer_add", [FName("add", 2)], [], [
      FunDef(
        FName("add", 2),
        CFun(
          ["a", "b"],
  ...
  assert atom.to_string(mod) == "carder@test@printer_add"
...
pub fn integration_factorial_recursion_test() {
  ...
  let m =
    CModule("carder@test@printer_fac", [FName("fac", 1)], [], [
...
pub fn integration_classify_guards_test() {
  ...
  let m =
    CModule("carder@test@printer_classify", [FName("classify", 1)], [], [
