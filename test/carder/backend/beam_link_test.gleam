...
/// `test/carder_emit_test_ffi.erl`) — the same harness the emit e2e tests use.
...
@external(erlang, "carder_linker_test_ffi", "install_synth")
...
/// (rotate is not inlined), so it emits a `call 'carder@runtime@rt_num':'i32_rotl'(a, b)` and
...
pub fn smoke_differential_numerics_test() {
  let name = "carder@link@smoke_num"
...
/// R4 (must-pass): a closure reaching `carder@runtime@rt_simd:f32x4_add/2`,
/// which CAPTURES `fun carder@runtime@rt_num:f32_add/2` (a distinct Core node —
...
/// with NO residual `carder@runtime@rt_num` remote reference. `rt_num:f32_add`
...
pub fn fun_capture_is_first_class_test() {
  ...
  let core =
    synth_forms(
      ...
      "use",
      ["A", "B"],
      CCall(CAtom("carder@runtime@rt_simd"), CAtom("f32x4_add"), [
        CVar("A"),
        CVar("B"),
      ]),
    )
  ...
  assert string.contains(text, "'carder@runtime@rt_num__f32_add'/2 =")
  // the rewritten capture references the merged module + mangled name.
  assert string.contains(text, "'carder@runtime@rt_num__f32_add'")
  // no external `fun 'carder@runtime@rt_num':...` capture literal survives.
  assert !string.contains(text, "fun 'carder@runtime@rt_num'")
}
...
pub fn dce_drops_unreachable_test() {
  let name = "carder@link@dce"
  let core = forms_of(gen_cmod(num_module(name, [clz_fn(), rotl_fn()])))
  let assert Ok(#(_, text)) = beam_link.link_to_core(core, name, ambient())
  // i32_clz + i32_rotl ARE reached (defs present).
  assert string.contains(text, "'carder@runtime@rt_num__i32_clz'/1 =")
  assert string.contains(text, "'carder@runtime@rt_num__i32_rotl'/2 =")
  // an unreferenced runtime function is DCE'd out.
  assert !string.contains(text, "'carder@runtime@rt_num__f64_sqrt'/1 =")
}
...
pub fn deterministic_output_test() {
  let name = "carder@link@determinism"
  let core = forms_of(gen_cmod(num_module(name, [clz_fn(), add_fn()])))
...
pub fn d3a_allows_first_class_apply_test() {
  let name = "carder@link@firstclass"
...
pub fn fail_closed_missing_module_test() {
  let name = "carder@link@missing"
  let core =
    synth_forms(
      name,
      "f",
      ["X"],
      CCall(CAtom("carder@runtime@does_not_exist"), CAtom("g"), [CVar("X")]),
    )
  let assert Error(MissingClosureModule("carder@runtime@does_not_exist")) =
    beam_link.link_program(core, name, ambient())
}
...
pub fn unmergeable_on_load_module_test() {
  let _ =
    install_synth(
      "carder_link_onload_synth",
      "-module(carder_link_onload_synth).\n-on_load(init/0).\n"
        <> "-export([f/0]).\ninit() -> ok.\nf() -> 1.\n",
    )
  let name = "carder@link@onload"
  let core =
    synth_forms(
      name,
      "f",
      [],
      CCall(CAtom("carder_link_onload_synth"), CAtom("f"), []),
    )
...
pub fn malformed_core_yields_typed_error_test() {
  let core = synth_forms("carder@link@broken", "f", [], CVar("Unbound"))
  let result = beam_link.link_program(core, "carder@link@broken", ambient())
...
pub fn declared_name_mismatch_yields_typed_error_test() {
  let core = forms_of(gen_cmod(num_module("carder@link@actual", [add_fn()])))
  let assert Error(MalformedCore(_)) =
    beam_link.link_program(core, "carder@link@requested", ambient())
}
