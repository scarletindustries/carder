...
//// (`carder_bindings_ffi:compile_load_gleam`), and calls typed exports through the compiled
//// binding — asserting the native result equals BOTH the WASM-spec value AND the raw in-process
//// `.beam` oracle (`carder_threaded_test_ffi`). Coverage: a signed-int export, a multi-value
...
fn driver_source() -> String {
  "import carder_wasm_statemod_bindings as s
import carder_wasm_puremod_bindings as p
...
pub fn emits_three_named_files_test() {
  ...
  assert list.map(files, fn(f) { f.path })
    == [
      "carder_wasm_statemod_bindings.gleam",
      "carder_wasm_statemod_bindings_ffi.erl",
      "carder_wasm_statemod_bindings_README.md",
    ]
}
...
pub fn binding_module_name_differs_from_beam_atom_test() {
  ...
  assert string.contains(
    gleam_file.content,
    "@external(erlang, \"carder@wasm@statemod\", \"add\")",
  )
}
