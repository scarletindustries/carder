...
/// emitted `.core` names `carder@runtime@rt_mem_atomics` (not the base's stale `rt_mem`), and a
...
pub fn atomics_build_links_and_runs_test() {
  ...
  assert string.contains(core, "carder@runtime@rt_mem_atomics")
  assert !string.contains(core, "'carder@runtime@rt_mem':")
...
pub fn cli_to_core_portable_independently_invokable_test() {
  let assert Ok(text) =
    carder.run(["to-core", "--portable", golden <> "/add.ir"])
...
pub fn cli_emit_tier_atomics_links_module_test() {
  ...
  assert string.contains(text, "carder@runtime@rt_mem_atomics")
}
