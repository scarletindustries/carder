...
fn run_embed(fixture: String, export: String) -> embed.InvokeResult {
  let assert Ok(bytes) = ffi.read_file("test/carder/teavm/" <> fixture)
...
pub fn embed_compiles_sdk_guests_test() {
  let assert Ok(counter) = ffi.read_file("test/carder/teavm/counter_java.wasm")
  let assert Ok(_) = embed.compile(counter)
  let assert Ok(channel) = ffi.read_file("test/carder/teavm/channel_java.wasm")
...
pub fn compile_named_preserves_semantics_test() {
  let assert Ok(bytes) = ffi.read_file("test/carder/teavm/compute.wasm")
  let assert Ok(compiled) =
    embed.compile_named(bytes, "carder@wasm@renamed_compute")
...
pub fn compile_progress_reports_phases_test() {
  ...
  let assert Ok(compiled) =
    embed.compile_progress(bytes, "carder@wasm@progress", fn(pct, phase) {
...
pub fn compile_named_distinct_atoms_coexist_test() {
  let host = fn(_capability, _name, _args) { [] }
  let assert Ok(compute) = ffi.read_file("test/carder/teavm/compute.wasm")
  let assert Ok(memtest) = ffi.read_file("test/carder/teavm/memtest.wasm")
  let assert Ok(a) = embed.compile_named(compute, "carder@wasm@coexist_a")
  let assert Ok(b) = embed.compile_named(memtest, "carder@wasm@coexist_b")
