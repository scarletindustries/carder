%%% carder_linker_ffi — the whole-program Core-Erlang linker (Phase 11 · P11-03).
%%%
%%% Merges a generated (wasm-derived) module and its ENTIRE transitive
%%% carder/gleam dependency closure into ONE self-contained `.beam` whose only
%%% remaining remote calls are to the fixed OTP-ambient allowlist — every
%%% `carder@*`/`gleam@*` dependency merged in, dead code stripped, no ambient
%%% authority. It is the same OTP-29-internals trust boundary as
%%% `carder_codegen_ffi.erl` (decision D10 / R1); it reuses the compiler's own
...
%% `carder/backend/eaf` emits); `ModuleName` is the merged module's atom name
