...
////    `iface.module_name` (`twocore@wasm@<x>` → `twocore_wasm_<x>_bindings`), so it never
...
///
/// `<base>` is `iface.module_name` legalized to a Gleam identifier (`twocore@wasm@x` →
/// `twocore_wasm_x`), so the binding module name can never collide with the loaded `.beam`
...
fn render_gleam(desc: Iface, shim_module: String) -> String {
  ...
  let header = [
    "//// Typed Gleam bindings for the compiled WASM module `"
      <> module_name
      <> "`.",
    "//// Generated by 2core — do not edit. Drop this file AND its companion `"
      <> shim_module
      <> ".erl`",
...
fn render_readme(
  ...
  [
    ...
    "",
    "Generated by 2core. Two source files make up the binding:",
    "",
    "- `"
      <> gleam_module
    ...
    "discovers the `.erl` FFI tree-wide under `src/`). The compiled `"
      <> desc.module_name
      <> ".beam`",
    "and the 2core runtime modules must be on the code path; for a self-contained artifact, compile",
