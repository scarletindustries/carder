...
//// `twocore@*`/`gleam@*` dependency closure into ONE self-contained `.beam`
...
//// orchestration over the `cerl` surgery in `src/twocore_linker_ffi.erl`; the
//// AST work lives in the FFI (the same OTP-29-internals trust boundary as
//// `build_beam`/`twocore_codegen_ffi`), and this module is the stable
...
////
//// PINNED TO OTP 29 (via the FFI). See `src/twocore_linker_ffi.erl` for the
...
/// FFI into `twocore_linker_ffi:link_program/3`. Returns the shim's
...
/// FFI into `twocore_linker_ffi:link_to_core/3` — the same merge returning the
