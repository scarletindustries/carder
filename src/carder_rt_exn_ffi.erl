%%% carder_rt_exn_ffi — the build-fixed WASM-exception term shim for `rt_exn`
%%% (Phase-7, J1/J5; T3/T6/T7/T9).
%%%
%%% Hand-written Erlang, so it carries the `carder_` namespace prefix (overview
%%% §5) and can NEVER collide with an OTP module — exactly like
%%% `carder_rt_ref_ffi`/`carder_rt_state_ffi`. Pure term construction /
