...
/// An unrecognised pair returns the empty result (a `()`-typed import carder does not model), never a
...
pub fn dispatch(
  ...
  case capability, name {
    ...
    // Any TeaVM host import carder does not (yet) model: a `()`-result no-op, fail-soft.
    _, _ -> fn(_args) { [] }
  }
}
