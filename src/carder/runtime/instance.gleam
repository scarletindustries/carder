...
////   reference resolves to a fixed, build-controlled `carder@runtime@*` module.
...
//// `carder/runtime/rt_num` maps to the Erlang module `carder@runtime@rt_num`
...
import carder/opt_level.{type OptLevel, Baseline}
...
/// `"carder@runtime@rt_num"`). `emit_core` emits `call '<field>':'<fn>'(...)` against
...
pub type Binding {
  Binding(
    ...
    /// functions. carder lowers every wasm block/if/switch continuation to a
    ...
    /// (e.g. a JS engine's opcode-dispatch loop, ~40+ locals per executed opcode). carder threads
...
/// the `own` stdlib, all wired to the default `carder@runtime@rt_*` modules.
///
...
pub fn safe_default() -> Binding {
  Binding(
    mode: Safe,
    num_module: "carder@runtime@rt_num",
    trap_module: "carder@runtime@rt_trap",
    host_module: "carder@runtime@rt_host",
    meter_module: "carder@runtime@rt_meter",
    stdlib_module: "carder@runtime@rt_stdlib",
    mem_module: "carder@runtime@rt_mem",
    table_module: "carder@runtime@rt_table",
    state_module: "carder@runtime@rt_state",
    ...
    js_runtime_module: "carder@runtime@rt_js",
