...
//// carrying the vetted `carder@runtime@rt_*` impl module names. A *linker* assembles a
...
//// names live in exactly one place — `instance.safe_default()` — so the `carder@runtime@*`
...
import carder/opt_level.{Aggressive}
import carder/runtime/instance.{
  type Binding, type DirectHost, type MemTier, type Mode, type StateStrategy,
  type TableTier, Atomics, BifOpen, Binding, Cell, HostOpen, HostWhitelist,
  MeterOff, Nif, Paged, Safe, StdlibPassthrough, TableAtomics, TableEts,
  TablePaged, Threaded, Unsafe, safe_default,
}
...
/// `carder@runtime@rt_*` impl module names (units 09/10) — memory/table/state layers wired
...
/// The **direct-IR frontend profile**: `Threaded` state plus the caller's `direct_host`
/// (capability + op table). `Some(direct_host)` is what makes `emit_core` thread the instance
/// state through every function/closure/throw/catch and dispatch each
/// `CallHost(host.capability, ...)` by its `OpKind`. `MeterOff` because metering is the
/// frontend runtime's job, not `ir_lower`'s `Charge` nodes. Tier-P on every axis (identical
/// to `portable()` bar `direct_host`/`meter`). Total.
pub fn direct(host: DirectHost) -> Binding {
  Binding(..portable(), direct_host: Some(host), meter: MeterOff)
}
...
/// `carder@runtime@rt_*` runtime module names as `safe()` (the runtime CODE is shared; the two
...
///   `carder@runtime@*` modules generated code will call; they are loaded into the build
...
///   (D1 — the existing vetted `carder@runtime@rt_mem` atom keeps its single home, never
...
pub fn mem_module_for(tier: MemTier) -> String {
  case tier {
    ...
    Atomics -> "carder@runtime@rt_mem_atomics"
    Nif -> "carder@runtime@rt_mem_nif"
  }
}
...
///   `safe_default().table_module` (D1 — the vetted `carder@runtime@rt_table` atom keeps its
...
pub fn table_module_for(tier: TableTier) -> String {
  case tier {
    ...
    TableEts -> "carder@runtime@rt_table_ets"
    TableAtomics -> "carder@runtime@rt_table_atomics"
  }
}
