...
//// `CarderRtState`, which compiles to the unique, namespace-hygienic atom
//// `carder_rt_state` (exactly the `rt_meter` `CarderRtMeterFuel → carder_rt_meter_fuel`
...
/// Read this process's cell with a sound presence check (the `carder_`-namespaced shim,
/// mirroring `carder_codegen_ffi`/`carder_cli_ffi`). Returns `Ok(state)` when the key
...
@external(erlang, "carder_rt_state_ffi", "read_cell")
...
/// Gleam constructor it compiles to the unique, namespace-hygienic atom `carder_rt_state`,
...
pub fn clear() -> Nil {
  let _ = erlang_erase(CarderRtState)
  Nil
}
...
fn put_cell(state: InstanceState) -> Nil {
  let _ = erlang_put(CarderRtState, state)
  Nil
}
