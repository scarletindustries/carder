...
///   module such as `is_ambient("carder@runtime@rt_num")` is False (in-closure ≠ ambient).
...
///
/// The linker (P11-03) implements the acquisition in `carder_linker_ffi.erl`; this manifest only
...
pub type Acquisition {
  ...
  /// `carder/backend/eaf`) lowered to `#c_module{}` via the compiler's own `to_core` pass
  ...
  /// needs no `.erl` on disk (verified on `rt_num`, `gleam@int`, `gleam_stdlib`, `carder_rt_exn_ffi`).
  ResidentBeamCore
...
pub fn frozen_runtime_roots() -> List(String) {
  sorted([
    ...
    "carder@runtime@rt_mem", "carder@runtime@rt_mem_atomics",
    "carder@runtime@rt_meter", "carder@runtime@rt_num", "carder@runtime@rt_ref",
...
/// `gleam_stdlib:identity` for D5 bit-identity coercions) and the 5 `carder_rt_*_ffi` shims;
...
/// `gleam_erlang_ffi` — this snapshot corrects it (7 members, not 6). `carder_rt_js_ffi` is EXCLUDED
/// (JS-only, off the WASM link path); `carder_cli_ffi`/`carder_codegen_ffi` are driver/compiler
...
pub fn frozen_ffi_erl() -> List(String) {
  sorted([
    "gleam_erlang_ffi", "gleam_stdlib", "carder_rt_exn_ffi",
    "carder_rt_mem_atomics_ffi", "carder_rt_ref_ffi", "carder_rt_state_ffi",
    "carder_rt_table_ets_ffi",
  ])
}
