...
/// identifier (`carder@wasm@x` → `carder_wasm_x`); the Elixir module compiles to the DISTINCT
/// atom `Elixir.<Camelized>Bindings`, so — unlike the Gleam/Erlang siblings' `carder_wasm_x_bindings`
...
/// (`carder@wasm@x` → `CarderWasmXBindings`). Compiles to the atom `Elixir.<alias>` — distinct
...
fn moduledoc_block(desc: Iface) -> List(String) {
  ...
  [
    ...
    "",
    "  Generated by carder (Phase 12) - do not edit. Values follow the WASM value-ABI",
...
/// (`carder_wasm_x` → `CarderWasmX`). The base starts with `[a-z]`, so the result starts with an
...
/// always quoted (`mod` is `carder@wasm@<base>` — the `@` requires it). The dispatch function name
