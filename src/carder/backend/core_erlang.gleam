...
//// Abstract Format terms (`carder/backend/eaf`) consumed in-process by
//// `compile:forms/2`; `carder/backend/core_printer` renders the same AST as
...
///
/// Fields:
/// - `name`: the module atom in logical form (e.g. `"carder@runtime@rt_num"`);
///   the printer single-quotes and escapes it. Namespacing (`carder@…`) is the
