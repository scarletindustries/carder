...
////
//// Wraps the hand-written Erlang shim `carder_codegen_ffi` (the «FFI-SHIM»)
...
//// (`carder/backend/eaf`), compiles them **in-process** with
...
@external(erlang, "carder_codegen_ffi", "compile_forms")
...
@external(erlang, "carder_codegen_ffi", "load_module")
fn ffi_load_module(
  module: Atom,
  filename: String,
  beam: BitArray,
...
@external(erlang, "carder_codegen_ffi", "forms_to_erl")
...
/// generated modules namespaced `carder@…` to avoid clobbering OTP or each
/// other.
pub fn load_module(
  module: Atom,
  filename: String,
...
///
/// The load `filename` is fixed to `"carder_generated"` (metadata only); use
...
pub fn compile_and_load(cmod: CModule) -> Result(Atom, BuildError) {
  ...
  load_module(mod, "carder_generated", beam)
