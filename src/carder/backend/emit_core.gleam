...
//// here and nowhere else, against the fixed `carder@runtime@*` module names carried by
...
/// The reference-value runtime module (`runtime/rt_ref` → `carder@runtime@rt_ref`, R1). The
...
const ref_module = "carder@runtime@rt_ref"

/// The non-function-import link/instantiate contract module (`runtime/link` →
/// `carder@runtime@link`, R4). Home for the `provided_*` externval extractors the generated
...
const link_module = "carder@runtime@link"

/// The SIMD lane-op runtime module (`runtime/rt_simd` → `carder@runtime@rt_simd`, I1/I2). The
...
/// else, I3/I8.)
const simd_module = "carder@runtime@rt_simd"
...
const gc_module = "carder@runtime@rt_gc"

/// The tagged-exception runtime module (`runtime/rt_exn` → `carder@runtime@rt_exn`, J1/T3).
...
/// `erlang:raise/3` live INSIDE `rt_exn`, never in generated code — the homogeneous carder-only
/// allow-set (S5, no `erlang` entry).
const exn_module = "carder@runtime@rt_exn"
...
/// - `binding`: the build-time runtime binding (the fixed `carder@runtime@*` module
...
/// `carder@…` namespacing is the caller's responsibility (overview §5).
pub fn emit_module(
  module: Module,
  binding: Binding,
  ...
  // module-unique offset so cerl_inline (enabled in carder_codegen_ffi)
          ...
          case target_threaded {
            ...
            True ->
              case captures {
                ...
                // per-jsf profile). carder_codegen_ffi catches any residual
                // beam_ssa_opt type-lattice crash and retries `no_type_opt`.
                // Spike: arc/test/emit_carder_cfunref_spike.gleam.
...
/// carder_codegen_ffi.erl now catches any residual OTP-internal crash and
...
/// bisect. Spike: arc/test/emit_carder_cfunref_spike.gleam.
    ...
    KBind(names, body, next) as kb ->
      ...
      case list.length(names) == arity {
        ...
        True ->
          ...
          // (carder_codegen_ffi) trips `cerl_inline` `undefined_var` on it.
          case
            force_inline
            || {
              is_direct(ctx.binding)
...
///
/// carder lowers each wasm block/if/switch continuation to a single-def
...
/// - SINGLE DEF: the letrec binds exactly one def (carder emits exactly one for joins/loops/try).
...
/// `(X band M2) band M1 == X band M2`. Every mask carder emits here — `2^n-1`, the shift counts
...
/// seam (E1). `module` is always a build-controlled `carder@runtime@*` atom (one of
...
/// static-type retag emitting no `num_module` call. This is bit-exact and total: carder carries
...
/// carder represents every scalar — floats as raw bits (D5) — as the same underlying Erlang
...
fn resolve_js_legacy(op: String) -> Option(String) {
  case op {
    // arithmetic (sentinel-aware IEEE; see rt_js / carder_rt_js_ffi)
    "add" -> Some("add")
    "sub" -> Some("sub")
    "mul" -> Some("mul")
    "neg" -> Some("neg")
