...
import carder/ir.{type Expr, type Function, type Module, type Value}
import carder/runtime/instance.{type Binding, MeterFuel, MeterOff}
import carder/runtime/rt_bif
...
/// `binding.stdlib_module` (a `carder@runtime@rt_*` module), never a raw BEAM module (F6/D3a).
///
...
/// `carder@runtime@rt_*` module) under BOTH postures. Exposed for the anti-drift cross-check
