%%% twocore_rt_mem_nif_ffi — the tier-N («nif») linear-memory NIF SHIM (S15-01 keystone).
%%%
%%% Hand-written Erlang under `src/`, so it carries the `twocore_` namespace prefix (overview §5,
%%% same convention as `twocore_rt_mem_atomics_ffi`) and can NEVER collide with an OTP module. Unlike
...
%%%
%%%   (a) ERL_NIF_INIT MODULE ATOM = THIS module = `twocore_rt_mem_nif_ffi`. `load_nif` attaches NIFs
%%%       to the module in whose code it runs, and `ERL_NIF_INIT`'s first token MUST name that same
%%%       module. So S15-02's `c_src/twocore_rt_mem_nif.c` declares
%%%       `ERL_NIF_INIT(twocore_rt_mem_nif_ffi, …)` — NOT `twocore_rt_mem_nif`. (Verified live by the
%%%       keystone build test: with the `_ffi`-suffixed atom, `load_nif` attaches and `nif_ping()`
%%%       dispatches to C.)
%%%
%%%   (b) THE `.so` BASENAME is `twocore_rt_mem_nif` (the file on disk is `twocore_rt_mem_nif.so`;
%%%       `load_nif`'s path arg is the basename WITHOUT the extension). `-on_load` resolves it via the
%%%       `TWOCORE_RT_MEM_NIF_SO` env override (the TEST-TIME load path — the build FFI sets it to the
%%%       freshly-compiled tempdir base, then force-reloads this module to re-run `-on_load`) then
%%%       `priv/twocore_rt_mem_nif` (the DEPLOYMENT path — a prebuilt per-platform `.so`, a documented
...
%%
%% Load path: the `TWOCORE_RT_MEM_NIF_SO` env override (test-time, set by the build FFI to the
%% freshly-compiled tempdir base) takes precedence, else `priv/twocore_rt_mem_nif` (deployment
%% follow-on). The path is a `.so` basename WITHOUT the extension (a `load_nif` convention).
%% Returns: `ok` (always — the soft-load decision).
init() ->
    Base =
        case os:getenv("TWOCORE_RT_MEM_NIF_SO") of
            false ->
                ...
                case code:priv_dir(twocore) of
                    {error, _} -> "twocore_rt_mem_nif";
                    Priv -> filename:join(Priv, "twocore_rt_mem_nif")
                end;
            Path ->
                Path
        end,
