...
| **R14-02** Backend + seed + driver (the heart, lockstep) | `done` | `«REFFUNC-IMPORT-FROZEN»` | Real imported-funcref emission (inline D3a adapter, Cell+Threaded; `render_ref_item`+`render_ref_global_init`+`emit_ref_func_import` arms; `all_reffunc` treats it as not-plain; `needs_func_imports` made **`pub`** + scans element segments) + driver `module_calls_import` **delegates** to it (single source of truth). **`table_copy.json` fully flipped 569/1080 → 1649/0/0** (headline 47,734/683/0, +1088). **ABI note (load-bearing):** the funcref-slot ABI is now **package-ABI** (Phase-13's `rt_table` re-wraps package→list), so the adapter reshapes `call_import`'s list into the `function_return` package — a raw-list adapter (stale pre-Phase-13 doc) would double-wrap/crash. 10 tests. |
| **R14-03** Runtime differential coverage | `done` | `«REFFUNC-IMPORT-FROZEN»` | Import-routed funcref slot proven bit-identical across all 6 `{TablePaged/TableEts/TableAtomics} × {Cell/Threaded}` combos, incl. `table.copy` both directions + the 3 ordered guards + package-ABI reshape for 0/1/N results. No cross-tier divergence. Pure test-only (+6 tests). |
