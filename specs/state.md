...
| **S15-04** Node-safety fuzz + full `cell_nif` matrix | `done` | S15-02 | C bounds-check fuzz (off-by-one @ grow watermarks, no-wrap + **memory64 `[2^64-32, 2^64-1]` overflow vectors**, all-or-nothing straddles for store/init_data/fill/copy/init/store_bytes, **cross-resource copy** proving both handles checked, post-grow zero-fill escape probe, boundary-biased nif≡paged≡oracle `to_flat` differential ×3 seeds) proving no host escape; the 4 Safe-forbidden gates + `--link` exclusion re-confirmed read-only; native `init_data` OOB bug FIXED (imported paged memory delegated). `combos.cell_nif` drives NATIVE across the corpus tier differential (conformance stays the paged delegate — see caveat). +16 tests. |
...
- S15-04 node-safety fuzz + native matrix — 2111 tests (was 2095, +16), conformance **47734/683/0** (unchanged — cell_nif's 3 imported-memory `data.wast` OOB cases now pass natively via the fix, same as the delegate), 0 warnings, format clean, default tier-P/O byte-identical. **Native `init_data` OOB fix:** an OOB active-data segment into an IMPORTED (paged) memory raised `badarg` (foreign handle → `enif_get_resource` fails); `rt_mem_nif`'s segment/bulk writers now discriminate on the handle shape (`is_native_mem` — a paged `Mem` is a tuple, a native resource is a reference) and DELEGATE imported paged handles to `rt_mem`, so the trap is a bit-identical `Error(MemoryOutOfBounds)`. Verified against the 3 real `data.wast` cases (lines 274/305/320) — `data.json` = 34/0/0 native. **Security fuzz** green against the real `.so` (cc present; the memory64 overflow vectors + cross-resource copy + boundary differential proved no host escape). **⚠ S15-05 must know (native-matrix ordering caveat):** the S15-01 keystone probe opens its resource type `ERL_NIF_RT_CREATE`-only (no `TAKEOVER`), so its shim reload FAILS while any nif resource is live; the conformance harness (`twocore_conformance_ffi:start_common`) spawns each instance in an UNLINKED orphan (`spawn`, trap-isolation) that holds its memory forever. So force-loading the `.so` during the PRE-probe conformance matrix permanently breaks the probe. Resolution (honest, `fail=0`): `combos.binding_for` REUSES the `.so` if already attached but never force-loads; the dedicated post-probe native tests (`rt_mem_nif_safety_test`/`rt_mem_nif_test`) attach it, so the **corpus tier differential runs native** (verified: 33 cell_nif bindings saw `nif_available=true`), while the conformance `cell_nif` point stays the byte-identical paged delegate (protecting the probe). The self-asserting native proofs are the security fuzz + the S15-02 differential.
