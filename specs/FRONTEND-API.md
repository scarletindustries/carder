# The carder frontend API — the IR contract every frontend emits

> **Audience: anyone writing a carder frontend.** This is the **complete, authoritative
> interface** between a frontend and carder: the value model, the calling convention, every IR
> node you can emit, the compiled-code↔runtime ABI, and how to lower common constructs. You never
> touch `emit_core`/`build_beam` — you produce a `carder/ir.Module` and hand it to the existing
> pipeline (`pipeline.compile_ir` / `pipeline.run_ir` → `ir_lower` → `ir_opt` → `emit_core` →
> `build_beam` → BEAM). **carder owns the IR and everything below it; you own your source
> language, your emitter, and your language runtime.**
>
> Frontends today: **scribbler** (WebAssembly — `scarletindustries/scribbler`) and **arc**
> (JavaScript — `alii/arc`). Neither has any code in the carder repo, and carder has no code in
> theirs. That separation is the point: it is what lets a frontend ship on its own cadence.
>
> The worked example throughout is the **JavaScript** frontend, because JS exercises the widest
> part of the IR (closures, maps, term tests, native arithmetic, exceptions). A frontend for a
> statically-typed or lower-level language uses a strict subset — the WebAssembly frontend, for
> instance, uses the numeric/memory/table layer and almost none of the term layer.

## What carder gives you, in one screen
...
// 1. build a module (this doc is the reference for what you may put in it)
let module: ir.Module = my_emitter(source)

// 2. compile it
pipeline.compile_ir(module, profiles.safe())        // -> Result(BitArray, PipelineError)

// 3. or compile AND run it
pipeline.run_ir(module, profiles.safe(), "main", [])  // -> Result(RunResult, PipelineError)

// 4. if your language has a runtime, bind it (no carder change needed to add an op)
pipeline.compile_ir(module, profiles.direct(DirectHost("js", ops)))

// 5. if your source imports host modules carder has never heard of, supply them
link.Namespace("teavmJso", resolve_func, resolve_state)   // -> pass in `providers`

// 6. reuse carder's CLI vocabulary in your own binary, so postures never drift
cli.with_binding(tokens, fn(binding, axes, positionals) { ... })
cli.axes_usage()
```

Everything else in this document is the detail behind step 1.
