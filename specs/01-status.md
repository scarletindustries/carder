...
carder is a **working** multi-frontend compiler that lowers WebAssembly into a shared, language-neutral
...
| JS-on-BEAM (Porffor 0.61.13 → carder → BEAM) | **52 pass / 0 fail / 3 skip** over a 55-program corpus (all 3 skips are Porffor's own `-0`/closure bugs, reproduced byte-for-byte — they bound Porffor, not carder) |
...
- **Consequence:** to load/run an emitted `.beam`, the **compiled `carder` OTP application** must be on
  ...
  which works because that VM *is* the running `carder` app and already has the runtime loaded.
...
loaded in-process (D10), which works because that VM *is* the running `carder` app.
...
with no `carder@*`/`gleam@*` beams reachable. `--link` runs the same compile pipeline, then a
whole-program **Core Erlang merge** (`beam_link.link_program` over the `cerl` FFI `carder_linker_ffi.erl`,
...
exit). Example: `gleam run -- run test/carder/conformance/corpus/add.wasm add 3 5` → `8`.
...
```
src/carder.gleam                         CLI dispatch (arg parsing + file IO only)
src/carder/pipeline.gleam                top-level driver glue; per-stage error mapping
...
src/carder/frontend/wasm/decode.gleam    untrusted .wasm bytes → AST
src/carder/frontend/wasm/wat.gleam       WASM text-format frontend (lexer + parse_module/parse_script)
src/carder/frontend/wasm/validate.gleam  full WASM validation — the security boundary
src/carder/frontend/wasm/lower.gleam     validated WASM → shared IR (stack-elim/SSA)
src/carder/frontend/wasm/ast.gleam       decoded WASM module model
...
src/carder/backend/build_beam.gleam      .core text → in-memory .beam, loaded into the VM
...
src/carder/runtime/link.gleam            fail-closed instantiation/import contract
...
src/carder/runtime/rt_mem_nif.gleam      linear memory — nif (tier-N; skeleton; Unsafe-only)
...
src/carder/runtime/rt_host.gleam         per-instance capability boundary for host imports
...
src/carder/runtime/rt_js.gleam           JS runtime boundary — STUB (real impl is the frontend team's)
src/carder/runtime/porffor_abi.gleam     pure Porffor (f64,i32) typed-value ABI
```
