/*
 * carder_rt_mem_nif.c — the tier-N («nif») linear-memory C core (S15-02, the heart).
 *
 * A real `erl_nif` backend over a RESERVED raw byte buffer managed by an ERTS resource, filling
 * the 16 exports frozen by S15-01 (`c_src/carder_rt_mem_nif.h` + `src/carder_rt_mem_nif_ffi.erl`).
 * It is BIT-IDENTICAL to the paged reference `carder/runtime/rt_mem` for every access — the S15-02
 * per-op `nif ≡ paged ≡ oracle` differential (test/carder/runtime/rt_mem_nif_test.gleam) is the
...
static int get_mem(ErlNifEnv *env, ERL_NIF_TERM t, carder_mem_t **out) {
...
static ERL_NIF_TERM nif_fresh(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    ...
    carder_mem_t *m = enif_alloc_resource(MEM_RES_TYPE,
                                           sizeof(carder_mem_t) + (size_t)max_bytes);
...
static int load(ErlNifEnv *env, void **priv_data, ERL_NIF_TERM load_info) {
    ...
    ErlNifResourceType *rt = enif_open_resource_type(
        env, NULL, CARDER_RT_MEM_NIF_RESOURCE, NULL,
...
/* The module atom is the SHIM (`carder_rt_mem_nif_ffi`), NOT the .so basename (keystone §3.3): load_nif
