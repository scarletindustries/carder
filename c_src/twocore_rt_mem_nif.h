/*
 * twocore_rt_mem_nif.h — the FROZEN tier-N («nif») linear-memory resource ABI.
 *
 * OWNED BY S15-01 (the keystone). This header is the single frozen surface that BOTH the C core
 * (c_src/twocore_rt_mem_nif.c, written by S15-02) AND the Gleam heads
 * (src/twocore/runtime/rt_mem_nif.gleam, re-bodied by S15-02) bind to. The keystone DEFINES the
 * layout + the per-operation contract; it implements NONE of the ops (only the throwaway `nif_ping`
 * probe, compiled from a source embedded in test/twocore_rt_mem_nif_build_ffi.erl, ever touches it
...
#ifndef TWOCORE_RT_MEM_NIF_H
#define TWOCORE_RT_MEM_NIF_H
 ...
 *                `sizeof(twocore_mem_t) + max_bytes` via `enif_alloc_resource`.
 */
typedef struct {
 ...
 * (`size`/`init`/`copy`/`fill`). The `ERL_NIF_INIT` module atom is `twocore_rt_mem_nif_ffi` (the
 * shim module), NOT `twocore_rt_mem_nif` — `load_nif` attaches NIFs to the module in whose code it
 * runs, and the Gleam @externals target `"twocore_rt_mem_nif_ffi"`.
 *
